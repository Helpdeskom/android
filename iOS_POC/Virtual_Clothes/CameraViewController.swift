...
//
    ...
    class CameraViewController: UIViewController {
     ...
     func detectPose(in image: MLImage, width: CGFloat, height: CGFloat) {
      ...
      private func updatePreviewOverlayViewWithImageBuffer(_ imageBuffer: CVImageBuffer?) {
        ...
        func showSubImage(){
            ...
            print(widthPose)
            print(heightPose)
            let imgExtra = UIImageView(frame: CGRect(x: shoulderX - 40, y: shoulderY - 30, width: widtChesthPose + 80, height: heightChestPose + 80))
            //(frame: CGRect(x: imageX - 60, y: imageY - 100, width: widthPose, height: heightPose))
                   print(imgExtra)
            imgExtra.tag = 100
...
//           }
            if let image = UIImage(named: "YellowTshirt") {
                ...
                print("Image not found or nil")
            }

            for subView in previewOverlayView.subviews {
                if subView.tag == 100 {
                    subView.removeFromSuperview()
                    break
                }
            }
            self.previewOverlayView.addSubview(imgExtra)
