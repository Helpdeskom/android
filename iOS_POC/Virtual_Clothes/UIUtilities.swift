...
var imageX = CGFloat()
var imageY = CGFloat()
var sholderWidth = CGFloat()
var sholderToHipHeight = CGFloat()
var heightPose: CGFloat = 0.00
var widthPose: CGFloat = 0.00

var leftx: CGFloat = 0.00
var rightx: CGFloat = 0.00

var lefty: CGFloat = 0.00
var righty: CGFloat = 0.00
...
var widtChesthPose = CGFloat()
var heightChestPose = CGFloat()
...
public class UIUtilities {
  ...
  public static func createPoseOverlayView(
    ...
    let lowerBodyHeight: CGFloat =
      ...
      let upperBodyHeight: CGFloat = UIUtilities.distance(
        fromPoint: pose.landmark(ofType: PoseLandmarkType.leftShoulder).position,
        toPoint: pose.landmark(ofType: PoseLandmarkType.rightShoulder).position)
      + UIUtilities.distance(
        fromPoint: pose.landmark(ofType: PoseLandmarkType.leftHip).position,
        toPoint: pose.landmark(ofType: PoseLandmarkType.rightHip).position)
      print(upperBodyHeight)
      
      let leftSolderToleftHip : CGFloat = UIUtilities.distance(
        fromPoint: pose.landmark(ofType: PoseLandmarkType.leftShoulder).position,
        toPoint: pose.landmark(ofType: PoseLandmarkType.leftHip).position)
      print("leftsholder to lefthip \(leftSolderToleftHip)")
      print(pose.landmark(ofType: PoseLandmarkType.leftShoulder).position)
      print(pose.landmark(ofType: PoseLandmarkType.rightShoulder).position)
      sholderToHipHeight = leftSolderToleftHip - 33
      heightPose = leftSolderToleftHip
      
      let rightSolderTorighttHip: CGFloat = UIUtilities.distance(
        fromPoint: pose.landmark(ofType: PoseLandmarkType.rightShoulder).position,
        toPoint: pose.landmark(ofType: PoseLandmarkType.rightHip).position)
      print(rightSolderTorighttHip)
      
      let leftSolderTorighttSholder : CGFloat = UIUtilities.distance(
        fromPoint: pose.landmark(ofType: PoseLandmarkType.leftShoulder).position,
        toPoint: pose.landmark(ofType: PoseLandmarkType.rightShoulder).position)
      print("left sholder to right sholder\(leftSolderTorighttSholder)")
      sholderWidth = leftSolderTorighttSholder + 40
      widthPose = leftSolderTorighttSholder
      
      let leftHipToRightHip: CGFloat = UIUtilities.distance(
        fromPoint: pose.landmark(ofType: PoseLandmarkType.leftHip).position,
        toPoint: pose.landmark(ofType: PoseLandmarkType.rightHip).position)
      print(leftHipToRightHip)

      // Inside your function, after calculating shoulder positions
      if let leftShoulder = leftShoulder, let rightShoulder = rightShoulder {
          let averageShoulderX = (leftShoulder.x + rightShoulder.x) / 2
          let averageShoulderY = (leftShoulder.y + rightShoulder.y) / 2

          let chestWidth = widthPose
          let chestHeight = heightPose

          // Calculate the top-left corner coordinates for the image
          let chestImageX = averageShoulderX - chestWidth / 2
          let chestImageY = averageShoulderY - chestHeight / 2
          
          print(chestImageX, "+++++chest x coordinate +++")
          print(chestImageY, "+++++chest y coordinate +++")
          print(chestWidth, "=====chest width value ====")
          print(chestHeight, "=====chest height value ====")
          
          shoulderX = chestImageX
          shoulderY = chestImageY
          widtChesthPose = chestWidth
          heightChestPose = chestHeight
    ...
    let farZColor = UIColor.blue
      ...
      for (startLandmarkType, endLandmarkTypesArray) in UIUtilities.poseConnections() {
          ...
          print(startLandmarkType.rawValue, startLandmarkType.hashValue , startLandmark.position.x, startLandmark.position.y ,"start pose ======")
          
          for endLandmarkType in endLandmarkTypesArray {
              
              let endLandmark = pose.landmark(ofType: endLandmarkType)
              
              print(endLandmarkType.rawValue, endLandmarkType.hashValue , endLandmark.position.x , endLandmark.position.y, endLandmark.position.z,"End pose ======")
              ...
              print(startLandmarkType.rawValue,  "====positionssss")
              print(startLandmarkPoint, startLandmarkType.rawValue, "1 ====positionssss"  )
              
              if startLandmarkType.rawValue == "LeftShoulder" {
                  leftShoulder = startLandmarkPoint
                  print(leftShoulder, "LeftShoulder ====positionssss")
              } else if startLandmarkType.rawValue == "RightShoulder" {
                  rightShoulder = startLandmarkPoint
                  print(rightShoulder, "RightShoulder ====positionssss")
              }
          }
      }
      if let leftShoulder = leftShoulder, let rightShoulder = rightShoulder {
          ...
          let averageRightShouldery = (rightShoulder.y)
          let averageRightShoulderx = (rightShoulder.x)
          
          imageX = averageRightShoulderx
          imageY = averageRightShouldery
          print("Average Shoulder X: \(averageShoulderX), Y: \(averageShoulderY)")
          
      } else {
          print("Left or right shoulder landmark not found.")
      }

      
//    for (startLandmarkType, endLandmarkTypesArray) in UIUtilities.poseConnections() {
//      let startLandmark = pose.landmark(ofType: startLandmarkType)
//        print(startLandmarkType)
//        print(startLandmark)
//        print(endLandmarkTypesArray)
//
//      for endLandmarkType in endLandmarkTypesArray {
//          print(endLandmarkType)
//        let endLandmark = pose.landmark(ofType: endLandmarkType)
//        let startLandmarkPoint = positionTransformationClosure(startLandmark.position)
//        let endLandmarkPoint = positionTransformationClosure(endLandmark.position)
//
...
//          fromColor: nearZColor, toColor: farZColor, ratio: connectedLandmarkZRatio)
//
////          let leftSholderX: CGFloat =  startLandmarkType.rawValue == "LeftShoulder" ? startLandmarkPoint.x : 0
////          let leftSholderY: CGFloat =  startLandmarkType.rawValue == "LeftShoulder" ? startLandmarkPoint.y : 0
////          print(leftSholderX)
////          print(leftSholderY)
//
//          if startLandmarkType.rawValue == "RightShoulder" {
//              let rightSholderX: CGFloat =  startLandmarkPoint.x
//              let rightSholderY: CGFloat =  startLandmarkPoint.y
////              imageX = rightSholderX
////              imageY = rightSholderY
//          }
//
//      //    if startLandmarkType.rawValue == "LeftShoulder" || startLandmarkType.rawValue == "RightShoulder"{
//
//              if startLandmarkType.rawValue == "LeftShoulder" {
//                  leftx  =  startLandmarkPoint.x
//                  print(leftx, "lef===x ===axissssss")
//              } else if  startLandmarkType.rawValue == "RightShoulder" {
//                  rightx =  startLandmarkPoint.x
//                  print(rightx, "reg===x ===axissssss")
//              }
//
//              let shoulderX = (leftx + rightx) / 2
//              print(shoulderX, "===x ===axissssss")
//              imageX = shoulderX
//      //    }
//
//
...
//
//              if startLandmarkType.rawValue == "LeftShoulder" {
//                  lefty =  startLandmarkPoint.y
//                  print(lefty, "lef===y ===axissssss")
//              } else if startLandmarkType.rawValue == "RightShoulder"  {
//                  righty =  startLandmarkPoint.y
//                  print(righty, "reg===y ===axissssss")
//              }
//              let poseY = (lefty + righty) / 2
//          print(poseY,  "===y ===axissssss")
//              imageY = poseY
//         // }
//
//
//
////          let rightSholderX: CGFloat =  startLandmarkType.rawValue == "RightShoulder" ? startLandmarkPoint.x : 0
////          let rightSholderY: CGFloat =  startLandmarkType.rawValue == "RightShoulder" ? startLandmarkPoint.y : 0
////          print(rightSholderX)
////          print(rightSholderY)
//
//
